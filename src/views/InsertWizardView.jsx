...
// =============================================================================
// Se ADMIN: mostra tutti i club nella tendina
// Se REFERENTE: mostra solo i club associati all'utente (userClubs)
//
// COLLEGAMENTI:
// - Riceve isAdmin, userClubs, userProfile da App.jsx
// - Usa Navbar per navigazione
// - Usa ProgressRing per anello punteggio
// - Usa toast per errori di salvataggio
// =============================================================================
...
export default function InsertWizardView({ isAdmin, userClubs, userProfile, resolvedTheme, ThemeSwitcher, toast }) {
  ...
  useEffect(() => {
    async function fetchInitialData() {
      ...
      // Se admin: carica tutti i club. Se referente: filtra per club associati
      if (isAdmin) {
  ...
  return (
    ...
    >
      ...
      {/* Step indicator sotto la navbar */}
      <div className="flex justify-center gap-2 sm:gap-3 py-4 border-b border-slate-200 dark:border-white/5 bg-white/50 dark:bg-white/5">
        {[1, 2, 3].map((s) => (
          <div
            key={s}
      ...
      <div className="flex-1 flex flex-col lg:flex-row overflow-hidden">
        {/* Sidebar con ProgressRing (nascosta nello step 1) */}
        <aside
            ...
            {step === 1 && (
              <div className="space-y-10 sm:space-y-12 animate-in fade-in slide-in-from-right-8 duration-500">
                ...
                <div className="space-y-8 bg-white dark:bg-white/5 p-12 rounded-[3.5rem] border border-slate-200 dark:border-white/10 shadow-2xl">
                  {/* Tendina Club (filtrata per referente) */}
            ...
            {step === 2 && (
              <div className="space-y-12 animate-in fade-in slide-in-from-right-8 duration-500">
                      ...
                      <div key={rule.id} className="p-8 bg-white dark:bg-white/5 rounded-[3rem] border border-slate-200 dark:border-white/10 hover:border-brand-blue/30 transition-all shadow-md group">
                        <div className="flex justify-between items-start mb-6">
                          <div className="flex flex-col">
                            <span className="text-[10px] font-black uppercase text-slate-500 tracking-widest mb-1">{p?.nome}</span>
                        ...
                        <input
                          ...
                          className={`w-full bg-transparent text-5xl font-black outline-none border-b-4 transition-all pb-2 cursor-text text-brand-blue dark:text-white ${
                            err ? 'border-brand-red text-brand-red' : 'border-slate-100 dark:border-white/10 focus:border-brand-blue'
                          }`}
                        />
                        {err && (
                ...
                <div className="flex gap-4 fixed bottom-8 left-1/2 -translate-x-1/2 w-full max-w-xl px-6 z-50">
                  ...
                  <button
                    onClick={() => setStep(3)}
                    disabled={Object.keys(formErrors).length > 0 || !allFieldsFilled || isSubmitting}
            ...
            {step === 3 && (
              <div className="space-y-12 animate-in fade-in zoom-in duration-500 text-center">
                <div className="space-y-4">
                  ...
                  <p className="text-slate-500 dark:text-slate-400 max-w-sm mx-auto font-medium text-base italic">Verifica i parametri acquisiti.</p>
                </div>
                <div className="bg-white dark:bg-white/5 p-14 rounded-[4rem] border border-slate-200 dark:border-white/10 shadow-2xl space-y-10 relative overflow-hidden">
                  ...
                  <div className="flex flex-col sm:flex-row justify-between items-center gap-6 pb-8 border-b border-slate-100 dark:border-white/5 relative z-10 text-balance text-brand-dark dark:text-white">
                    <div className="text-center sm:text-left">
                      <div className="text-[10px] font-black text-slate-500 uppercase tracking-widest mb-2">Club</div>
                  ...
                  <div className="grid grid-cols-2 sm:grid-cols-4 gap-4 relative z-10 px-2 text-brand-dark dark:text-white">
                    {currentRules.map((rule) => (
                      <div key={rule.id} className="text-center p-4 bg-slate-50 dark:bg-black/30 rounded-2xl border border-slate-200 dark:border-white/5 shadow-inner">
                        <div className="text-[8px] font-black text-slate-500 dark:text-slate-500 uppercase mb-2 truncate px-1">{parameters.find((p) => p.id === rule.id_parametro)?.nome}</div>
                ...
                <div className="flex flex-col gap-6 max-w-sm mx-auto pb-10 px-4">
                  ...
                  <button onClick={() => setStep(2)} className="text-slate-500 dark:text-slate-400 font-black uppercase text-xs hover:text-brand-blue dark:hover:text-brand-yellow transition-colors cursor-pointer active:scale-90 p-2 underline underline-offset-8">
                    Modifica Parametri
                  </button>
                </div>
              </div>
