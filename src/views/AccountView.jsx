...
//
// COLLEGAMENTI:
// - Usa supabase.auth.updateUser per cambio password
// - Usa supabase.storage per upload avatar
// - Usa Navbar per navigazione
// =============================================================================

import { useState } from 'react';
import { Camera, Lock, CheckCircle2, Activity } from 'lucide-react';
...
export default function AccountView({ isAdmin, userProfile, ThemeSwitcher, toast }) {
  ...
  // ===========================================================================
  // handleChangePassword: Aggiorna la password via Supabase Auth
  // ===========================================================================
  ...
  // ===========================================================================
  // handleAvatarUpload: Carica l'immagine su Supabase Storage
  // 1. Carica nel bucket 'avatars' con nome univoco
  // 2. Ottiene l'URL pubblico
  // 3. Salva l'URL nella tabella utenti
  // ===========================================================================
  const handleAvatarUpload = async (e) => {
    ...
    setIsUploading(true);
    try {
      // Nome file univoco basato sull'user ID
      const fileExt = file.name.split('.').pop();
      const fileName = `${userProfile.id}.${fileExt}`;

      // Upload su Supabase Storage (sovrascrive se esiste)
      ...
      // Salva l'URL nel profilo utente
    ...
    <div className="min-h-screen bg-slate-100 dark:bg-[#0B132B] transition-colors duration-500">
      ...
      <div className="max-w-2xl mx-auto p-6 sm:p-12 space-y-8 text-brand-dark dark:text-white">
        ...
        <div className="bg-white dark:bg-white/5 p-8 rounded-[2.5rem] border border-slate-200 dark:border-white/10 shadow-xl">
          <h2 className="text-sm font-black uppercase tracking-widest text-brand-blue dark:text-brand-yellow mb-6">
            Immagine Profilo
          </h2>
          <div className="flex items-center gap-6">
            ...
            <div className="relative">
              {avatarUrl ? (
                <img
                  src={avatarUrl}
                  alt="Avatar"
                  className="w-24 h-24 rounded-[1.5rem] object-cover border-2 border-brand-blue dark:border-brand-yellow"
                />
              ) : (
            ...
            <div>
              ...
              {isUploading && (
                <div className="flex items-center gap-2 mt-2 text-xs text-brand-blue">
                  <Activity size={14} className="animate-spin" /> Caricamento...
                </div>
              )}
            </div>
          </div>
          ...
          <form onSubmit={handleChangePassword} className="space-y-4">
            ...
            >
              {isChangingPassword ? (
                ...
                <>
                  <Lock size={18} /> Aggiorna Password
                </>
              )}
            </button>
          </form>
        ...
        <div className="bg-white dark:bg-white/5 p-8 rounded-[2.5rem] border border-slate-200 dark:border-white/10 shadow-xl">
          ...
          <div className="space-y-3 text-sm">
            ...
            <div className="flex justify-between">
              ...
              <span className="font-black uppercase text-brand-blue dark:text-brand-yellow">
                {userProfile?.ruolo || 'referente'}
              </span>
            </div>
          </div>
        </div>
